...
class SerialPanel(ttk.LabelFrame):
    ...
    def refresh_ports(self):
        ...
        ports_list = [p.device for p in ports]   # Solo extraemos el nombre (COMx / /dev/ttyUSBx)

        current = self.port_var.get()            # Puerto que tenía seleccionado antes del rescan

        self.combo_ports["values"] = ports_list  # Reemplaza la lista del desplegable

        # Mantiene la selección previa si sigue disponible
        if current in ports_list:
            self.port_var.set(current)
        elif ports_list:
            self.combo_ports.current(0)          # Selecciona el primero de la nueva lista
        else:
