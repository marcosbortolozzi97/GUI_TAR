...
class PanelHistograma(ttk.LabelFrame):
    ...
    def _build_canal(self, canal, row):
        ...
        # MIN y MAX (solo controlan el ZOOM visual, no filtran datos)
        vars_ = {}
        ...
        # Feedback temporal
        status = ttk.Label(cfg, text="", foreground="green", font=("Arial", 8))
    ...
    def _aplicar_cfg(self, canal, vars_):
        ...
        if vmax > ADC_MAX:
            messagebox.showerror("Error", f"MAX no puede superar {ADC_MAX} (límite del ADC)")
            return
        ...
        # Guarda configuración visual del canal (solo zoom)
        self._cfg[canal] = {
            "min":       vmin,
            "max":       vmax,
            "intervalo": 1,
